...
public class AccessReviewExcelExporterTests
{
    ...
    private static XLWorkbook Generate(AccessReviewSnapshot snapshot, out string fileName)
    {
        ...
        var result = new AccessReviewExcelExporter().Generate("Cliente Demo", snapshot, accounts, kpis, findings, [], AccessReviewDelta.Empty, 90);
        fileName = result.FileName;
        return new XLWorkbook(new MemoryStream(result.Bytes));
    }

    [Fact]
    public void Genera_nueve_hojas_con_datos()
    {
        using var wb = Generate(Snapshot(), out var fileName);

        Assert.EndsWith(".xlsx", fileName);
        Assert.Equal(9, wb.Worksheets.Count);
    ...
    public void Hoja_de_hallazgos_lista_severidad_y_recomendacion()
    {
        ...
        // 19 reglas + encabezado: los limpios también se listan (saber que se evaluó es información).
        Assert.Equal(20, ws.LastRowUsed()!.RowNumber());
