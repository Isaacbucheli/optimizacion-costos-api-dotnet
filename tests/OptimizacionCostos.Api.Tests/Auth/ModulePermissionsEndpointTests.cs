...
public sealed class ModulePermissionsEndpointTests : IClassFixture<ModulePermissionsEndpointTests.Factory>
{
    ...
    [Fact]
    public async Task Me_de_admin_trae_los_11_modulos_todo_true()
    {
    ...
    public async Task Get_matriz_devuelve_catalogo_y_permisos_de_ambos_roles()
    {
        ...
        Assert.Equal(11, body.GetProperty("modules").GetArrayLength());
        Assert.Equal(11, body.GetProperty("permissions").GetProperty("consultor").GetArrayLength());
        Assert.Equal(11, body.GetProperty("permissions").GetProperty("lector").GetArrayLength());
