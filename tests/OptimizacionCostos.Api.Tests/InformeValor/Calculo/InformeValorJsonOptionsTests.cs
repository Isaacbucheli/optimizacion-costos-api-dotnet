...
public sealed class InformeValorJsonOptionsTests
{
    ...
    /// <summary>El modelo completo serializa con las DIEZ claves de nivel superior (entrega 6:
    /// <c>ejecutado</c> se suma a las siete que ya esperaba <c>render()</c>
    /// —<c>D.meta/.tickets/.fact/.rbac/.advisor/.matriz/.catSerie</c>—; entrega 7 Tarea 1: se suman
    /// <c>opex</c> y <c>cronologia</c>), ni una más ni una menos, y los bloques ausentes viajan como
    /// <c>null</c> JSON real, no un objeto vacío que simule ausencia (render() distingue "sin
    /// insumo" con <c>if(!t)</c>). <c>ejecutado</c>, <c>opex</c> y <c>cronologia</c> son las tres que
    /// <c>render()</c> todavía no lee (llegan en la entrega 7): acá solo se fija que viajan como
    /// clave propia, en null cuando el ensamblador no tuvo insumo con qué calcularlas.
    /// </summary>
    [Fact]
    public void El_modelo_completo_serializa_con_las_diez_claves_de_D()
    {
        ...
        Assert.Equal(
            new HashSet<string> { "meta", "tickets", "fact", "rbac", "advisor", "matriz", "catSerie", "ejecutado", "opex", "cronologia" },
            claves);
        ...
        Assert.Equal(JsonValueKind.Null, doc.RootElement.GetProperty("opex").ValueKind);
        Assert.Equal(JsonValueKind.Null, doc.RootElement.GetProperty("cronologia").ValueKind);
    }
